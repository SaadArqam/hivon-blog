...
export default function PostCard({ post, onDelete, likeCount, liked }: PostCardProps) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      // Verification check from API response
      if (data.success || data.deleteData) {
        toast.success('Story deleted successfully.')
        onDelete?.()
      } else {
        toast.error('Deletion result was inconclusive.')
      }
    } catch (err) {
  ...
  return (
    <>
      ...
      <ConfirmationModal
        ...
        loading={deleting}
        title="Remove Story?"
        description="This action will permanently delete this story and all its comments. This cannot be undone."
        confirmText="Remove"
        variant="destructive"
      />
    </>
